...
mergeSort:
	...
	cmp rsi, rdx		; if (right >= left), jmp
	jge mergeSortDone
	mov rcx, rdx 		; mid = (left + right) / 2
	add rcx, rsi
	shr rcx, 1		

	push rsi 			; storing left
	push rdx			; storing right
	push rcx 			; storing mid

	; at this point, (2)rsi = left, (3)rdx = right, (4)rcx = mid

	mov rdx, rcx 		; param 3 is mid
	call mergeSort		; recursive call on: arr, left, mid

	pop rcx				; reset params
	pop rdx
	pop rsi
	...
	mov rsi, rcx 		; param 2 is mid
	add rsi, 1 			; mid += 1
	call mergeSort 		; recursive call on: arr, mid+1, right
	...
	mov r8, rcx  		; mid -> r8
	mov rcx, rdx 		; right -> rcx
	mov rdx, r8 		; mid -> rdx

	; at this point, (2)rsi = left, (3)rdx = mid, (4)rcx = right
