...
int main(int argc, char** argv) {
    ...
    // TODO: YOUR CODE HERE
    string start = dests.front();
    dests.erase(dests.begin());
    float minTime = computeDistance(me, start, dests);
    vector<string> minDests = dests;

    sort(dests.begin(), dests.end());

    do {
        if (computeDistance(me, start, dests) < minTime) {
            minTime = computeDistance(me, start, dests);
            minDests = dests;
        }
    } while (next_permutation(dests.begin(), dests.end()));

    cout << "Minimum path has distance " << minTime << ": ";
...
float computeDistance(MiddleEarth& me, const string& start, vector<string> dests) {
    ...
    distance += me.getDistance(start, dests.front());
